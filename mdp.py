...
class Map:
    ...
    ## helper function for valueIteration()
    def calculateUtility(self, state):
        expectedUtility = 0
        for action in state.actions:
            expectedUtility = state.computeEU()
        newUtility = state.reward + expectedUtility
        changeUtility = state.utility - newUtility
    ...
    def valueIteration(self):
        ...
        while True:
            for state in self.states:
                if not state.isGoal:
                    change = self.calculateUtility(state)
                if change > greatestChange:
                    greatestChange = change
            if greatestChange < self.stop_crit:
                break
